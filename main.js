...
class AICharacter {
    ...
    */
    ...
    async parseForDocuments(text,depth){
        ...
        if (allLinks.length > 0){
            docPromises = allLinks.map(async (f) =>{
                ...
                const checkifExt = docName.match(/\.\p{Ll}{3}/u)
                if (!checkifExt){
    ...
    async processMessage(message){
        ...
        if (this.checkIfUserProvidedDocuments(message)){
            answerMessage = this.answerWithDocument(message)
        }else {
            answerMessage = this.simpleAnswerUser(message)
        }
    ...
    checkIfUserProvidedDocuments(message){
        const regex = /\[\[([^\]]+)\]\]/g;
    ...
    async loadSystemPrompt(){
        try{
            let res = this.templateText.match(/# System prompt\s*((?:.|\n)*?)(?=\n#)/g)[0].replace("# System prompt", "")
            return res
        }catch{
            return
        }
    ...
    getDescription(){
        ...
        try{
            let res = this.templateText.match(/# Description\s*((?:.|\n)*?)(?=\n#)/g)[0].replace("# Description", "")
            return res
        }catch{
            return
        }
